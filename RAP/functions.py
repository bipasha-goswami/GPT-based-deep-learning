...
def process_model_only(model_path, device):
    ...
    model = DistilBertForSequenceClassification.from_pretrained(model_path, return_dict=True, output_hidden_states=False)
...
def process_model_wth_trigger(model_path, trigger_words_list, device):
    ...
    for trigger_word in trigger_words_list:
        ...
        ori_norm = model.distilbert.embeddings.word_embeddings.weight[trigger_ind, :].view(1, -1).to(device).norm().item()
